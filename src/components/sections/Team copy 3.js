...
const Team = () => {
  ...
  const fetchTokenData = async (fullTokenIdApi, fullCreatorContract) => {
    try {
      ...
      throw new Error('Erro ao buscar dados na API: ' + error.message);
    }
  };
        ...
        smartContract.methods.getMintedTokenInfoAll().call()
          .then(tokens => {
             ...
             // console.log('creatorContract:', creatorContract);
              ...
              console.log('tokenID:',fullTokenIdApi, 'contrato',fullCreatorContractApi);
                            
              // Chamar a função fetchTokenData e acessar os dados retornados dentro do then 0x07006DdFAFE43606E1282A18122b7458402cD0EB
          fetchTokenData(fullTokenIdApi, fullCreatorContractApi)
            // fetchTokenData('4','0xa16aa825602e276392b5833a2bc6bf607d33c747') 
              .then(data => {
                console.log('## Buscando vendas do contrato:', data.length > 0 ? data[0].from : 'Nenhum dado encontrado');
                 data.forEach(item => {
                  ...
                  console.log('tokenValue:', item.tokenValue);
...
console.log('to', to)
              return {
                ...
                tokenValue: `Qte tokens comprados: ${tokenValue}`,
                to: `Comprador: ${to}`,                           
                creatorContractHash: (
                  <a
