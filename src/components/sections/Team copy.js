        ...
        smartContract.methods.getMintedTokenInfoAll().call()
          .then(tokens => {
            const newCards = tokens.projectNames.map((name, index) => {
              ...
              const shortenedHash = `${fullHash.slice(0, 6)}...${fullHash.slice(-6)}`;
              const fullCreatorContract = tokens.creatorContracts[index]; // Adicionando creatorContracts   
              const creatorContract = `${fullCreatorContract.slice(0, 6)}...${fullCreatorContract.slice(-6)}`; 

              return {
                img: 'https://via.placeholder.com/200',
                title: name || 'Nome do projeto',
                tokenId: `TokenID: ${tokens.tokenIds[index]}`, // Adicionando tokenIds
                creatorContractHash: (
                  <a
