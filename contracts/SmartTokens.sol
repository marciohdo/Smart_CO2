...
//startedate  1675257200000 = Wed 20 September 2023 21:46:40 (data-type is "Number")
//Endedate    1695257200000 = Wed  1 February  2023 10:13:20 (data-type is "Number")
// A variavel "amount" eu subistui pela var = CCT para indicar a quantide de Tokens de que contrato vai disponibilidar.

contract SmartTokens is ERC1155, Ownable {
...
function mint(uint256 CCT, 
    ...
    require(CCT > 0, "Quantidade de tokens deve ser maior que zero");
    require(startDate < endDate, "Datas inicio deve ser menor que a de termino");
    require(bytes(projectName).length > 0, "Nome do projeto nao pode ser vazio");
    require(bytes(ipfsHash).length > 0, "Hash IPFS nao pode ser vazio");
    require(startDate != 0 && endDate != 0, "informe date de inicio e termino");
   ...
   // Adiciona logs para debug
    ...
    // Adicione esse evento no seu contrato
    event MintEvent(
